...
const renderDashboard = async () => {
  ...
  el.innerHTML = `
    <div class="card kpi-card"><div class="kpi-value">${state.students.length}</div><div class="kpi-label">Students</div></div>
    <div class="card kpi-card"><div class="kpi-value">${periods}</div><div class="kpi-label">Periods</div></div>
    <div class="card kpi-card"><div class="kpi-value">N/A</div><div class="kpi-label">Avg. Mastery</div></div>
...
const renderRoster = () => {
  ...
  container.innerHTML = `
  <table>
    <thead><tr><th>Name</th><th>ID</th><th>Quarter</th><th>Period</th><th>Test</th><th>Score</th></tr></thead>
    <tbody>
      ${state.students.map(s=>`
        <tr>
          <td>${s.name}</td><td>${s.id}</td><td>${s.quarter}</td><td>${s.period}</td>
          <td>${s.test || 'N/A'}</td><td>${s.score || 'N/A'}</td>
        </tr>`).join('')}
    </tbody>
  </table>`;
};
...
const showCrash = (msg) => {
  ...
  details.textContent = String(msg ?? 'Unknown error');
  overlay.hidden = false;
};
window.addEventListener('error',   e => showCrash(e?.error?.stack || e?.message || e));
window.addEventListener('unhandledrejection', e => showCrash(e?.reason?.stack || e?.reason || 'Unhandled Promise rejection'));
